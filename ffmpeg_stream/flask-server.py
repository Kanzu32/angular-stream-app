...
def crop_post():
    ...
    if section_mode:
        ...
        file.close()
        os.system(f"ffmpeg -y -ss {timestamp} -i ./recorded/{in_file} -c:v libx264 -pix_fmt yuv420p -force_key_frames source -x264-params keyint=25:scenecut=0 ./recorded/fixed.mp4")
        file_path = os.path.join(root, "recorded", "fixed.mp4")
    else:
        begin = options["begin"]
        end = options["end"]
        os.system(f"ffmpeg -y -i ./recorded/{in_file} -vcodec libx264 -pix_fmt yuv420p -ss {begin} -to {end} ./recorded/crop.mp4")
...
@app.route('/dash-stream/<string:filename>', methods=['GET'])
def dash_stream(filename):
    try:
        file_path = os.path.join(root, "dash-stream", filename)
        print(file_path)
        if os.path.isfile(file_path):
            return send_file(file_path, as_attachment=True)
        else:
